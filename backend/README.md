...
параметрам запроса. Три объяснения запрашиваются параллельно; каждый вызов
ограничен 5,5 секунды, при тайм-ауте возвращается короткий текст из проверенных
фактов. Используется `text-embedding-3-small`.
